...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, num_group=32):
        ...
        if planes == 64:
            self.globalAvgPool = nn.AvgPool2d(32, stride=1)#56
        elif planes == 128:
            self.globalAvgPool = nn.AvgPool2d(16, stride=1)#28
        elif planes == 256:
            self.globalAvgPool = nn.AvgPool2d(8, stride=1)#14
        elif planes == 512:
            self.globalAvgPool = nn.AvgPool2d(4, stride=1)#7
...
class SE_ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_group=32):
        ...
        self.fc1 = nn.Linear(512 * block.expansion, 500)
        self.dropout = nn.Dropout(p=0.5)
        self.fc2 = nn.Linear(500,21)
